...
import voicevox_request_local_api


class App:
    def __init__(self, master):
        # 対話処理システムの準備
        self.qa_system = qa_chatbot_uec_ced.UECQueryEngine(False)
        # 音声合成システムの準備
    ...
    def transcribe_audio(self):
        while self.continue_recording.is_set():
            try:
                ...
                # 対話処理結果をテキストボックスに追加する
                qa_answer = str(self.qa_system.query(text))
                print("qua_answer:", qa_answer)
                self.text_box.insert(tk.END, qa_answer + '\n')
                # 音声を作成する
                self.voicevox_system.make_sound_file(qa_answer)
                self.voicevox_system.play_sound_file()
            except queue.Empty:
                continue
